...
class EMA:
    ...
    """
    def update(self, model):
        # Ensure we're working with the core model in case it's wrapped with DataParallel
        if hasattr(model, 'module'):
        ...
        with torch.no_grad():
            ...
            for ema_key, ema_val in self.ema.state_dict().items():
                # Adjust the key to match the model's state_dict keys
                model_key = ema_key.replace('module.', '') if 'module.' in ema_key else ema_key
                if model_key in model_state_dict:
                    model_val = model_state_dict[model_key]
                    # Manually ensure the type matches, as `ema_val` is a tensor reference in the EMA model's state_dict
                    ema_val.copy_(decay * ema_val + (1. - decay) * model_val.detach())
