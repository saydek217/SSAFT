...
class Dataset(data.Dataset):
    ...
    #images = [item[0] for item in batch]  # Extracting image tensors
    #targets = [item[1] for item in batch]  # Extracting target tensors

    # Pad the targets to the maximum number of annotations in the batch
    #max_annotations = max(target.shape[0] for target in targets)
   # padded_targets = []
   # for target in targets:
    #    if target.shape[0] < max_annotations:
    #        padding = torch.zeros((max_annotations - target.shape[0], target.shape[1]))
   ...
   # targets = torch.stack(padded_targets, dim=0)
    ...
    def load_label(filenames):
        ...
        x = {}
        for filename in tqdm.tqdm(filenames,  desc="Loading dataset"):
            try:
                # verify images
